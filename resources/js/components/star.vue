<template>
    <svg class="vue-star-rating-star" :height="getSize" :width="getSize" :viewBox="viewBox" @mousemove="mouseMoving" @click="selected">
        ...
        <linearGradient :id="grad" x1="0" x2="100%" y1="0" y2="0">
            <stop :offset="getFill" :stop-color="(rtl) ? inactiveColor : activeColor" ></stop>
            <stop :offset="getFill" :stop-color="(rtl) ? activeColor : inactiveColor" ></stop>
        </linearGradient>

        <filter :id="glowId"  height="130%" width="130%" filterUnits="userSpaceOnUse">
            <feGaussianBlur :stdDeviation="glow" result="coloredBlur"></feGaussianBlur>
            <feMerge>
        ...
        <polygon :points="starPointsToString" :fill="getGradId" :stroke="glowColor"
                 :filter="'url(#'+this.glowId+')'" v-show="fill > 1" ></polygon>

        <polygon :points="starPointsToString" :fill="getGradId" :stroke="getBorderColor" :stroke-width="border" :stroke-linejoin="roundedCorners ? 'round' : 'miter'" ></polygon>
        <polygon :points="starPointsToString" :fill="getGradId" ></polygon>
    </svg>
</template>
